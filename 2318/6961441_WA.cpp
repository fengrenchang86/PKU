...
int GetAns(int i, int j)
{
	...
	{
		double d1 = (double)toy[i].y - line[j].k * (double)(toy[i].x) - line[j].b;
		double d2 = (double)(y1+y2)/2.0 - line[j].k * (double)(x1) - line[j].b;
		if (d1 * d2 > 0)
		{
			return 1;
		}
...
void Solve()
{
	...
	for (i = 0; i < n; i++)
	{
		...
		else
		{
			...
			line[i].b = (double)y2 - line[i].k * (double)part[i].y;
		}
	}
	int p = 0;
	for (i = 0; i < m; i++)
