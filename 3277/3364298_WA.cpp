...
void update ( int i, int begin, int end, int x, int y, int high )
{
	...
	if ( begin == x && end == y )
	{
		if ( t[i].area < high*(b[end]-b[begin]) )
		{
			...
			if ( end-begin != 1 )
			{
				if ( t[i*2+1].area < high*(b[end]-b[mid]) )
				{
					t[i*2+1].area = high*(b[end]-b[mid]);
					t[i*2+1].cover = high;
				}
				if ( t[i*2].area < high*(b[end]-b[mid]) )
				{
					t[i*2].area = high*(b[end]-b[mid]);
					t[i*2].cover = high;
				}
				t[i*2].cover = high;
			}
	...
	if ( t[i].cover != 0 )
	{
		if ( t[i].area < t[i].cover*(b[end]-b[begin]) )
			t[i].area = t[i].cover*(b[end]-b[begin]);
		if ( t[i*2].area < t[i*2].cover*(b[mid]-b[begin]) )
		{
			t[i*2].cover = t[i].cover;
			t[i*2].area = t[i*2].cover*(b[mid]-b[begin]);
		}
		if ( t[i*2+1].area < t[i*2+1].cover*(b[end]-b[mid]) )
		{
			t[i*2+1].cover = t[i].cover;
			t[i*2+1].area = t[i*2+1].cover*(b[end]-b[mid]);
		}
		t[i].cover = 0;
	}
	if ( y <= mid )
