...
void update ( int i, int begin, int end, int x, int y, int high )
{
	...
	if ( begin == x && end == y )
	{
		...
		{
			t[i].maxh = high;
			t[i].area = __int64(high*__int64(b[end]-b[begin]));
			t[i].cover = high;
			return;
		}
		else if ( end-begin == 1 )
		{
			if ( t[i].area < __int64(high*__int64(b[end]-b[begin])) )
				t[i].area = __int64(high*__int64(b[end]-b[begin]));
			return;
		}
	}
	...
	if ( t[i].cover != 0 )
	{
		...
		t[i*2].area = __int64(t[i*2].cover*__int64(b[mid]-b[begin]));
		t[i*2+1].area = __int64(t[i*2+1].cover*__int64(b[end]-b[mid]));
