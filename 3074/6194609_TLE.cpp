	...
	while (c)
	{
		...
		for (i = 0; i < 9; i++)
		{
			for (j = 0; j < 9; j++)
			{
				...
				for (k = 1; k <= 9; k++)
				{
					if (m_kArrRow[i].bCanFill[k] == false && m_kArrCol[j].bCanFill[k] == false && m_kArrBlock[y].bCanFill[k] == false)
					{
						z = k;
						x++;
					}
				...
				if (x == 1)
				{
					m_kArrBlock[y].bCanFill[z] = true;
					m_kArrRow[i].bCanFill[z] = true;
					m_kArrCol[j].bCanFill[z] = true;
					m_kSudokuMarix[i][j].nValue = z;
					c = 1;
					v++;
				}
			}
