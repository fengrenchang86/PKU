...
//			printf("%d %d\n",a[i],b[i]);
				...
				for ( k = 0; k <= 800; k++ )
				{
					...
					if ( k-(a[i]-b[i]+M) >= 0 && c[(i+1)%2][j-1][k-(a[i]-b[i]+M)] == true )
					{
						...
						if ( c[(i+1)%2][j][k] == false )
						{
							...
							x[i%2][j][k] = x[(i+1)%2][j-1][k-(a[i]-b[i]+M)]+a[i]+b[i];
						}
						else if ( x[i%2][j][k] < x[(i+1)%2][j-1][k-(a[i]-b[i]+M)]+a[i]+b[i] )
						{
							d[i%2][j][k] = d[(i+1)%2][j-1][k-(a[i]-b[i]+M)]+pow[i];
							x[i%2][j][k] = x[(i+1)%2][j-1][k-(a[i]-b[i]+M)]+a[i]+b[i];
						}
		...
		{
			if ( abs(j-m*M) > abs(k-m*M) )
				pf(d[n%2][m][k]);
			else if ( abs(j-m*M) < abs(k-m*M) )
				pf(d[n%2][m][j]);
			else
			{
