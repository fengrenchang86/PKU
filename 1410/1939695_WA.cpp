...
void main ()
{
	...
	for ( i = 0; i < n; i++ )
	{
		...
		if ( (x[0]<x1||x[0]>x2)&&(x[1]<x1||x[1]>x2)&&(y[0]<y2||y[0]>y1)&&(y[1]<y2||y[1]>y1) )
			cout<<'F'<<endl;
		else if ( pbx>=x[1]&&pbx<=x[2]&&pby<=y[1]&&pby>=y[2] )
			cout<<'T'<<endl;
		else if ( pex>=x[1]&&pex<=x[2]&&pey<=y[1]&&pey>=y[2] )
			cout<<'T'<<endl;
		else if ( pex<=x[1]&&pbx>=x[2] || pbx<=x[1]&&pex>=x[2] )
			cout<<'T'<<endl;
		else if ( pey<=y[2]&&pby>=y[1] || pby<=y[2]&&pey>=y[1] )
			cout<<'T'<<endl;
		else
			cout<<'F'<<endl;
	}
