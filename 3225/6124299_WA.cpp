...
ShowAns();
	...
	Insert(1, 1, n, 1, 5, 1, true, true);
	ShowAns();

	Insert(1, 1, n, 3, 3, 0, true, true);
	ShowAns();

	Reverse(1, 1, n, 2, 4, true, true);
	ShowAns();

	Reverse(1, 1, n, 1, 5, true, false);
	ShowAns();
	Insert(1, 1, n, 5, n, 0, true, true);
	ShowAns();

	Insert(1, 1, n, 1, 2, 0, true, true);
	ShowAns();
	Insert(1, 1, n, 3, n, 0, true, true);
	ShowAns();
*/
	int i;
	char ch;
	...
	while (scanf(" %c %s", &ch, &str) != EOF)
	{
		...
		if (ch == 'U')
		{
			Insert(1, 1, n, xx, yy, 1, ix, iy);
		}
		else if (ch == 'I')
		{
			if (1 <= xx)
			{
				Insert(1, 1, n, 1, xx, 0, true, !ix);
			}
			Insert(1, 1, n, yy, 65536, 0, !iy, true);
		}
		else if (ch == 'D')
		{
			Insert(1, 1, n, xx, yy, 0, ix, iy);
		}
		else if (ch == 'C')
		{
			if (1 <= xx)
