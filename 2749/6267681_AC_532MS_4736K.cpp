...
vector<int>vecList[1100], vecReverseList[1100];
struct DATASTRUCT
{
	int s1, s2;
...
{
	vecList[x].push_back(y);
	vecReverseList[y].push_back(x);
	return;
	NODE *p = new NODE;
	p->v = y;
	p->next = pList[x];
...
void dfs_Topo(int node)
{
	visit[node] = 1;
	int i;
	for (i = 0; i < vecList[node].size(); i++)
	{
		if (!visit[vecList[node][i]])
		{
			dfs_Topo(vecList[node][i]);
		}
	}
	nCount++;
	t[nCount] = node;
...
void dfs_SCC(int node)
{
	...
	int i;
	for (i = 0; i < vecReverseList[node].size(); i++)
	{
		if (arrScc[vecReverseList[node][i]] == 0)
		{
			dfs_SCC(vecReverseList[node][i]);
		}
	}
	return;
...
bool Solve(int s)
{
	...
	Topo();
	memset(arrScc, 0, sizeof(arrScc));
	nCount = 1;
	flag = true;
	for (i = 2 * n - 1; flag && i >= 0; i--)
	{
		if (arrScc[t[i]] == 0)
		{
			dfs_SCC(t[i]);
