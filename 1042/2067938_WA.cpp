...
int main ()
{
	int n,hour,f[30],d[30],t[30],fish[30],go[30],ans[30];
	...
	while ( scanf("%d",&n) && n!=0 )
	{
		...
		for ( i = 1; i < n; i++ )//assume John go to one,two,....n lakes to fish
		{
			...
			while ( k > 0 )
			{
				...
				for ( j = 0; j <= i; j++ )
				{
					if ( f[j]-d[j]*fish[j] > f[time]-d[time]*fish[time] )
						time = j;
				}
				if ( f[time]-d[time]*fish[time] <= 0 )
					break;
				number += (f[time]-d[time]*fish[time]);
				fish[time]++;
				k--;
			}
			j = 0;
