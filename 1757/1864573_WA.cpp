...
void main ()
{
	...
	for ( i = 0; i < n; i++ )
	{
		...
		if ( (x[0]<x1||x[0]>x2)&&(x[1]<x1||x[1]>x2)&&(y[0]<y2||y[0]>y1)&&(y[1]<y2||y[1]>y1) )
			cout<<'F'<<endl;
		else if ( (x[0]>=pex&&x[0]<=pbx || x[0]<=pex&&x[0]>=pbx)&&
			(y[0]>=pey&&y[0]<=pby || y[0]<=pey&&y[0]>=pby) )
			cout<<'T'<<endl;
		else if ( (x[1]>=pex&&x[1]<=pbx || x[1]<=pex&&x[1]>=pbx)&&
			(y[1]>=pey&&y[1]<=pby || y[1]<=pey&&y[1]>=pby) )
			cout<<'T'<<endl;
		else if ( pex>=x1&&pex<=x2&&pey>=y2&&pey<=y1 )
			cout<<'T'<<endl;
		else
			cout<<'F'<<endl;
	}
