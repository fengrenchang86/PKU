...
__int64 f ( int nbegin, int nend )
{
	...
	if ( mid > 0 )
	{
		...
		if (!( mid+2 == n || a[mid-1] <= nx && nx <= ny && ny <= a[mid+2] ))
		{
			y = f(nbegin,mid)+f(mid+1,nend);
			a[mid] = nx;
			a[mid+1] = ny;
	...
	if ( mid+2 < n)
	{
		k = abs(nx-a[mid+1])+abs(ny-a[mid+1]);
		a[mid] = a[mid+1] = a[mid+1];
		if (!( mid == 0 || a[mid-1] <= nx && nx <= ny && ny <= a[mid+2] ) && a[mid-1] != a[mid+1])
		{
...
__int64 g ( int nbegin, int nend )
{
	...
	if ( mid > 0 )
	{
		k = abs(nx-a[mid-1])+abs(ny-a[mid-1]);
		a[mid] = a[mid+1] = a[mid-1];
		if (!( mid+2 == n || a[mid-1] >= nx && nx >= ny && ny >= a[mid+2] ) && a[mid-1] != a[mid])
		{
			y = g(nbegin,mid)+g(mid+1,nend);
			a[mid] = nx;
			a[mid+1] = ny;
	...
	if ( mid+2 < n)
	{
		k = abs(nx-a[mid+1])+abs(ny-a[mid+1]);
		a[mid] = a[mid+1] = a[mid+1];
		if (!( mid == 0 || a[mid-1] >= nx && nx >= ny && ny >= a[mid+2] ))
		{
			y = g(nbegin,mid)+g(mid+1,nend);
			a[mid] = nx;
			a[mid+1] = ny;
